...
class WatchQueue {
  ...
  async startWatch() {
    setImmediate(async () => {
      for (;;) {
        ...
        if (tx) {
          const { txHash, data, timestamp } = tx;
          try {
            ...
            if (!receipt) {
              ...
              if (Date.now() - timestamp > TIME_LIMIT) {
                if (this.statusCb) {
                  this.statusCb(STATUS_TIMEOUT, tx);
                }
              } else {
                // wait for retry
                setTimeout(() => this.queue.push(tx), TX_LOOP_INTERVAL);
              }
            } else if (this.statusCb) {
              if (Number.parseInt(receipt.status, 16) === 1) {
                this.statusCb(STATUS_SUCCESS, tx, receipt);
              } else {
                this.statusCb(STATUS_FAIL, tx, receipt);
              }
            }
          } catch (err) {
  ...
  add(doc) {
    const txHash = doc.id;
    const data = doc.data();
    this.queue.push({ timestamp: data.ts || Date.now(), txHash, data });
  }
  ...
  txRef.where('status', '==', 'pending')
    ...
    .onSnapshot((snapshot) => {
      snapshot.docChanges.filter(change => change.type === 'added')
        .forEach(change => watchQueue.add(change.doc));
    });
